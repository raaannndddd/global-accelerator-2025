...
export async function POST(request: NextRequest) {
  try {
    ...
    } else if (lowerCommand.includes('moon') || lowerCommand.includes('lunar') || lowerCommand.includes('crash')) {
      specialEvent = 'moon'
      isCatastrophic = true
    ...
    const isDeepseekSmall = model.includes('deepseek-r1:1.5b') || model.includes('deepseek-r1:7b')
    const prompt = `
...
${isQwen ? 'Return a valid JSON object with this structure:' : isDeepseekSmall ? 'IMPORTANT: Return ONLY a valid JSON object with this exact structure, no explanations or think tags:' : 'Return ONLY a valid JSON object with this exact structure:'}
{
...
${isDeepseekSmall ? `CRITICAL: The specialEvent field MUST be set to "${specialEvent || null}" exactly as shown.` : ''}
...
${isQwen ? 'Return only the JSON, no other text.' : isDeepseekSmall ? 'CRITICAL: Return ONLY the JSON object above, no explanations, no think tags, no code examples, just the JSON.' : ''}
`
    ...
    if (model.includes('deepseek-r1:1.5b') || model.includes('deepseek-r1:7b')) {
    ...
    if (!parsedResponse && (model.includes('deepseek-r1:1.5b') || model.includes('deepseek-r1:7b'))) {
      ...
      if (jsonStart !== -1 && jsonEnd > jsonStart) {
        ...
        try {
          ...
          console.log('JSON parse failed for extracted content:', extractedJson)
        }
      }
    }
    ...
    if (!parsedResponse) {
      ...
      if (commandLower.includes('car') || commandLower.includes('drive') || commandLower.includes('transport')) {
        co2Increase = 30
        toxicityIncrease = 8
        tempIncrease = 0.8
        pollutionIncrease = 5
      } else if (commandLower.includes('factory') || commandLower.includes('industry') || commandLower.includes('manufacture')) {
        co2Increase = 50
        toxicityIncrease = 15
        tempIncrease = 1.2
        pollutionIncrease = 10
      } else if (commandLower.includes('deforest') || commandLower.includes('cut') || commandLower.includes('tree')) {
        co2Increase = 40
        plantLoss = 50000000
        animalLoss = 10000000
        pollutionIncrease = 8
      } else if (commandLower.includes('pollute') || commandLower.includes('waste') || commandLower.includes('dump')) {
        toxicityIncrease = 20
    ...
    const createComparisonText = () => {
      ...
      if (validatedMetrics.humanPopulation !== currentMetrics.humanPopulation) {
        ...
        const direction = change > 0 ? '↑' : '↓'
        changes.push(`Humans: ${currentMetrics.humanPopulation.toLocaleString()} ${direction} ${validatedMetrics.humanPopulation.toLocaleString()}`)
      }
      ...
      if (validatedMetrics.animalPopulation !== currentMetrics.animalPopulation) {
        ...
        const direction = change > 0 ? '↑' : '↓'
        changes.push(`Animals: ${currentMetrics.animalPopulation.toLocaleString()} ${direction} ${validatedMetrics.animalPopulation.toLocaleString()}`)
      }
      ...
      if (validatedMetrics.plantPopulation !== currentMetrics.plantPopulation) {
        ...
        const direction = change > 0 ? '↑' : '↓'
        changes.push(`Plants: ${currentMetrics.plantPopulation.toLocaleString()} ${direction} ${validatedMetrics.plantPopulation.toLocaleString()}`)
      }
